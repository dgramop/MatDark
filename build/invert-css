...
<?php
// Hacked together script for inverting CSS colors, built specially for MatLight
//
// Build process:
// Build MatDark, Invert colors with this script, manually go in and check for everything working 
...
function inverseColors($css) {
    ...
    return str_ireplace("nobla", "black", str_ireplace("nowhi", "white", str_ireplace("white", "nobla", str_ireplace("black", "nowhi", $css))));
}
